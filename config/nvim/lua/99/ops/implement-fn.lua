...
--- @param text string?
--- @return string
local function format_title(text)
  if not text or text == "" then
    return "Implementing Function"
  end

  local line = text:gsub("\r", "")
  line = line:gsub("\n.*", "")
  line = line:gsub("^%s+", "")
  line = line:gsub("%s+$", "")
...
local function implement_fn(context)
  ...
  if vim.treesitter and vim.treesitter.get_node_text then
    local ok, text = pcall(vim.treesitter.get_node_text, fn_call, buffer)
    if ok then
      call_text = text
    end
  end
