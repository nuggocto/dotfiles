...
- Use `@test-quality/` when writing or reviewing tests. Prefer table-driven tests when a behavior has multiple cases.
...
- Use `@benchmark/` for performance claims. Benchmark the shipped binary or library with production-equivalent data, workload, and deployment topology.
- Use `@qa/` to exercise the shipped binary, server, or library integration as a real consumer; unit tests and `go test ./...` do not verify packaging, startup, migration, or deployment behavior.
