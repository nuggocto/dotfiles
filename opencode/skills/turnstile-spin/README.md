...
```sh
# OpenCode
mkdir -p ~/.config/opencode/skills/turnstile-spin && \
  curl -sSL https://developers.cloudflare.com/turnstile/spin.md \
  -o ~/.config/opencode/skills/turnstile-spin/SKILL.md
...
ln -s ~/.config/cloudflare-skills/turnstile-spin ~/.config/opencode/skills/turnstile-spin
```

For other agents, use the skill directory documented by that agent.
