...
> follow the matching installed SDK skill (`sentry-elixir-sdk`, `sentry-go-sdk`,
> `sentry-browser-sdk`, `sentry-svelte-sdk`, or `sentry-cloudflare-sdk`) or the official Sentry platform docs. This guide intentionally doesn't
> duplicate those setup instructions.
...
external recommender), wrap it in a custom span with `sentry_sdk.start_span` (see the official
Python SDK tracing docs for the full custom-span API):
