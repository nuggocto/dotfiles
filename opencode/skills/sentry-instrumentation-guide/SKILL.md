...
> [Feature Setup](../sentry-feature-setup/SKILL.md) > Instrumentation Guide
...
- **Install the SDK and turn on tracing, logs, and metrics** → the matching installed SDK skill
  (`sentry-elixir-sdk`, `sentry-go-sdk`, `sentry-browser-sdk`, `sentry-svelte-sdk`, or `sentry-cloudflare-sdk`). Each has per-feature
  reference files for tracing, logging, metrics, and more. For other platforms, use official Sentry docs.
